...
//        List<PayWalletMemberBind> payBindForwardList = iPayWalletMemberBindService.selectByMap(map);
//        if (payBindForwardList == null || payBindForwardList.size() == 0){
//          return   payRoleConfigService.findDefaultRole();
//        } else {
//            PayWalletMemberBind payBindForward = payBindForwardList.get(0);
//            //支付方角色
//           return payRoleConfigService.findOneById(payBindForward.getRoleId());
//        }
...
//        //支付方角色
//        PayRoleConfig forwardRole = null;
//        //收款方角色
//        PayRoleConfig receiveRole = null;

//        //查询转出方、接收方 钱包平台绑定关系  如果为空-抛异常
//        Map<String,Object> map =new HashMap<>();
//        map.put("member_id",fromMemberWallet.getMemberId());
//        List<PayWalletMemberBind> payBindForwardList = iPayWalletMemberBindService.selectByMap(map);
//        if (payBindForwardList == null || payBindForwardList.size() == 0){
//            forwardRole = payRoleConfigService.findDefaultRole();
//        } else {
//            PayWalletMemberBind payBindForward = payBindForwardList.get(0);
//            //支付方角色
//            forwardRole = payRoleConfigService.findOneById(payBindForward.getRoleId());
//        }

//        map.clear();
//        map.put("member_id",toMemberWallet.getMemberId());
//        List<PayWalletMemberBind> payBindReceiveList = iPayWalletMemberBindService.selectByMap(map);
//        if (payBindReceiveList == null || payBindReceiveList.size() == 0){
//            receiveRole = payRoleConfigService.findDefaultRole();
//        } else {
//            PayWalletMemberBind payBindReceive = payBindReceiveList.get(0);
//            //收款方角色
//            receiveRole = payRoleConfigService.findOneById(payBindReceive.getRoleId());
//        }

//        if (payBindForwardList.size() == 0 || payBindReceiveList.size() == 0){
//            return MessageRespResult.error(MessageCode.MEMBER_NOT_BIND);
//        }
...
//        mapRole.put("role_name",receiveRole.getRoleName());
//        mapRole.put("trade_unit",toMemberWallet.getCoin().getUnit());
//        List<PayRoleFeeRateConfig> receiveFeeList = iPayRoleFeeRateConfigService.selectByMap(mapRole);
...
//        if (receiveFeeList.size()!= 0){
//            if(receiveFeeList.size()>1){
////                return MessageRespResult.error(MessageCode.INVALID_FEE_CONFIG);
//                return MessageRespResult.error(500,msService.getMessage("INVALID_FEE_CONFIG"));
//            }
//            PayRoleFeeRateConfig receiveFee = receiveFeeList.get(0);
...
//                    payFastRecord.setComment();
                    ...
                    this.saveTransaction(fromMemberWallet,toMemberWallet,null,amount,receiveAmount,receiveFee.getIncomeFee(),payRecord.getId());

                }else{
                    //不同，获取价格
                    result = this.convertFee(receiveFee,receiveFee.getIncomeFee(),restTemplate);
                    if (result.getCode() != 0){
...
//                    payFastRecord.setComment();
                    ...
                    //交易记录
                    //支付钱包，收款钱包，手续费钱包，支付数额，收款数额，手续费，快速支付记录id
                    this.saveTransaction(fromMemberWallet,toMemberWallet,feeWallet,amount,amount,afterFee,payRecord.getId());
                }
            }else{
                //手续费类型：百分比
...
//                    payFastRecord.setComment();
                    ...
                    //交易记录
                    //支付钱包，收款钱包，手续费钱包，支付数额，收款数额，手续费，快速支付记录id
                    this.saveTransaction(fromMemberWallet,toMemberWallet,null,amount,receiveAmount,fee,payRecord.getId());
                }else{
                    //不同，获取价格
                    result = this.convertFee(receiveFee,fee,restTemplate);
                    if (result.getCode() != 0){
//                        return MessageRespResult.error(MessageCode.FAILED_COIN_PRICE);
                        return MessageRespResult.error(result.getCode(),result.getMessage());
                    }
...
//                    payFastRecord.setComment();
                ...
                //交易记录
                //支付钱包，收款钱包，手续费钱包，支付数额，收款数额，手续费，快速支付记录id
                this.saveTransaction(fromMemberWallet,toMemberWallet,null,amount,amount,BigDecimal.ZERO,payRecord.getId());
...
//        BigDecimal feeCoinPrice = new BigDecimal(0.1);
        ...
        if(tradeCoinPrice.compareTo(BigDecimal.ZERO) == 0 || feeCoinPrice.compareTo(BigDecimal.ZERO) == 0){
            return new MessageResult(500,msService.getMessage("PRICE_ERROR"));
        }
        //换算后的数额
        Coin coin = coinService.findByUnit(receiveFee.getFeeUnit());
    ...
    @WriteDataSource
    @Transactional(transactionManager = "transactionManager", rollbackFor = Exception.class)
    public void saveTransaction(MemberWallet fromMember,MemberWallet toMember,MemberWallet feeWallet,BigDecimal amount,BigDecimal receiveAmount,
        ...
        if (feeWallet == null) {
            ...
            if (fee.compareTo(BigDecimal.ZERO)>0) {
                ...
                toMemberTransactionFee.setSymbol(feeWallet.getCoin().getUnit());
                ...
                memberTransactionService.save(toMemberTransactionFee);
            }
        }
        memberTransactionService.save(fromMemberTransaction);
        memberTransactionService.save(toMemberTransaction);

    }

    public PayWalletService getService(){
        return SpringContextUtil.getBean(PayWalletService.class);
    }
    ...
    public MessageRespResult<PayFastRecord> platformExchange(MemberWallet fromMemberWallet, MemberWallet toMemberWallet,
        ...
        if (payRoleFeeRateConfig == null) {
            ...
            if (!payRoleFeeRateConfig.getTradeUnit().equals(payRoleFeeRateConfig.getFeeUnit())) {
                MessageResult result = this.convertFee(payRoleFeeRateConfig, fee, restTemplate);
                if (result.isSuccess()) {
...
//        payFastRecord.setComment();//备注
        ...
        // 记录资金记录
        getService().saveTransaction(fromMemberWallet, toMemberWallet, forward, amount, amount, fee, payFastRecord.getId());

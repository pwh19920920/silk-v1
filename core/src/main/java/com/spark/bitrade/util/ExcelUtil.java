...
public class ExcelUtil  {
    ...
    public static <T>  void   listToExcel (
            ...
            String sheetName,
            int sheetSize,
            OutputStream out) throws ExcelExportException{
     ...
     * @throws ExcelExportException
     */
    public static <T>  void   listToExcel (List<T> list , Field[] fields,int sheetSize, OutputStream out) throws ExcelExportException{
     ...
     * @return true
     */
    public static <T>  void   listToExcel (List<T> list , Field[] fields, OutputStream out,String className) throws ExcelExportException{
        ...
        try {
            ...
            for(int i=0; i<sheetNum; i++){
                //如果只有一个工作表的情况
                if(1==sheetNum){
                    WritableSheet sheet=wwb.createSheet(Class.forName(className).getAnnotation(ExcelSheet.class).name(), i);
                    fillSheet(sheet, list, fields, 0, list.size()-1);

                    //有多个工作表的情况
                }else{
                    WritableSheet sheet=wwb.createSheet(Class.forName(className).getAnnotation(ExcelSheet.class).name()+(i+1), i);
    ...
    private static <T> void fillSheet(
        ...
        for(int index=firstIndex;index<=lastIndex;index++){
            ...
            for(int i=0;i<fields.length;i++){
                ...
                //add|edit|del by  shenzucai 时间： 2018.06.20  原因：防止科学计数法
                if(objValue instanceof BigDecimal){
                    String fieldValue =((BigDecimal) objValue).toPlainString();
...
//            response.setContentType("application/octet-stream;charset=ISO8859-1");
    ...
     * @time 2018/9/5 17:32 
     */
    public static <T>  void  listToCSV (List<T> list , Field[] fields, HttpServletResponse response, String fileName) throws ExcelExportException{

        if(list.size()==0 || list==null){
            throw new ExcelExportException("数据源中没有任何数据");
        }
        setResponseHeader(response, fileName + ".csv");
        BufferedWriter csvWtriter = null;
        //创建工作簿并发送到OutputStream指定的地方
        WritableWorkbook wwb;
        try {
            // GB2312使正确读取分隔符","
            csvWtriter = new BufferedWriter(new OutputStreamWriter(response.getOutputStream(), "UTF-8"), 1024*1024);
            fillCSVSheet(csvWtriter, list, fields);
            csvWtriter.flush();
        }catch (Exception e) {
            e.printStackTrace();
            //如果是ExcelException，则直接抛出
    ...
     * @time 2018/9/5 17:32 
     */
    private static <T> void fillCSVSheet(
        ...
        for(int i=0;i<fields.length;i++){
            ...
            String rowStr = sb.append("\"").append(fields[i].getAnnotation(Excel.class).name()).append("\",").toString();
...
//                    String rowStr = sb.append("\"").append(fieldValue).append("\",").toString();
                    //edit by zyj: 时间： 2018.10.09  原因：防止长数字文本科学计数法
                    String rowStr = sb.append("\t").append(fieldValue).append("\t,").toString();
                    csvWriter.write(rowStr);
                }else {
                    String fieldValue = objValue == null ? "" : objValue.toString();

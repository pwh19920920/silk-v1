...
import static com.spark.bitrade.util.MessageRespResult.*;
...
//        }
    ...
    */
    ...
    public MessageRespResult<PayFastRecord> platformTransfer(Long fromMemberId, Long toMemberId, String tradeUnit, BigDecimal amount,
        ...
        //验证双方是否被禁用交易
        validateOpenTranscationService.validateOpenPlatformTransaction(fromMemberId,sourceService.getMessage("CURRENT_INTERNAL_TRANSFER_DISABLED"));
        validateOpenTranscationService.validateOpenPlatformTransaction(toMemberId,sourceService.getMessage("ADVERSE_ACCOUNT_DISABLE"));
        //转出账户
...
//            return error(MessageCode.ERROR);
    ...
    public MessageRespResult<PayFastRecord> platformTransferByPhone(String fromPhone, String toPhone, String tradeUnit, BigDecimal amount,
        ...
        validateOpenTranscationService.validateOpenPlatformTransaction(fromMember.getId(),sourceService.getMessage("CURRENT_INTERNAL_TRANSFER_DISABLED"));
        validateOpenTranscationService.validateOpenPlatformTransaction(toMember.getId(),sourceService.getMessage("ADVERSE_ACCOUNT_DISABLE"));
        //转出账户
...
//                return error(MessageCode.WALLET_GET_FAIL);
    ...
    //todo 该接口最新优化在pay模块（DCC现暂用此接口，后续需换成pay模块的此接口）
    public MessageRespResult<PayToMemberVo> toMemberInfo(String unit,String address,@SessionAttribute(SESSION_MEMBER) AuthMember user){
        ...
        PayWalletMemberBind bind=iPayWalletMemberBindService.findByMemberIdAndAppId(memberId,user.getPlatform());
        PayWalletPlatMemberBind platMemberBind = iPayWalletPlatMemberBindService.findByMemberIdAndAppId(memberId,user.getPlatform());

        PaySupportCoinConfig coinConfig = iPaySupportCoinConfigService.findByStatusAndUnitAndAppId(
                                                                                unit,BooleanEnum.IS_TRUE.getOrdinal(),user.getPlatform());
        ...
        if (bind == null){
            ...
            payToMemberVo.setIsFast(BooleanEnum.IS_TRUE);

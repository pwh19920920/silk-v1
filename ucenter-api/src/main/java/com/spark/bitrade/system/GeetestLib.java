...
public class GeetestLib {
     ...
     * @return
     */
    private String getFailPreProcessRes(boolean enabled) {
        ...
        try {
            ...
            //jsonObject.put("enabled", true); //add by yangch 时间： 2018.07.13 原因：是否启用极验证
     ...
     * 预处理成功后的标准串
     */
    private String getSuccessPreProcessRes(String challenge, boolean enabled) {
     ...
     */
    public int preProcess(HashMap<String, String> data, boolean enabled) {
        if (registerChallenge(data, enabled) != 1) {
            this.responseStr = this.getFailPreProcessRes(enabled);
            return 0;
        }
        return 1;
    }
     ...
     */
    private int registerChallenge(HashMap<String, String> data, boolean enabled) {
        try {
            ...
            if (return_challenge.length() == 32) {
                this.responseStr = this.getSuccessPreProcessRes(this.md5Encode(return_challenge + this.privateKey), enabled);
                return 1;
            } else {

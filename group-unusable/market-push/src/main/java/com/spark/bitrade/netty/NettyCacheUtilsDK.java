...
public class NettyCacheUtilsDK {
    ...
    public static Map<Channel, String> keyChannelCache = Collections.synchronizedMap(new HashMap()); //修改为线程安全
    ...
    public static void storeChannel(String key, Channel channel) {
        ...
        if (set == null) {
            Set<Channel> setNew = Collections.synchronizedSet(new HashSet()); //修改为线程安全
            setNew.add(channel);

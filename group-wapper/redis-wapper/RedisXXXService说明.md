...
## 介绍
&emsp;Java操作Redis的工具类，封装了对Redis五种基本类型的各种操作，力求符合Redis的原生操作！
...
## 用法
### 一、keys相关命令
类：com.spark.bitrade.service.optfor.RedisKeyService
...
### 二、String数据类型操作
类：com.spark.bitrade.service.optfor.RedisStringService
...
### 三、Hash相关的操作 
类名：com.spark.bitrade.service.optfor.RedisHashService
...
### 四、List相关的操作
类名：com.spark.bitrade.service.optfor.RedisListService
...
### 五、Set相关的操作
类名：com.spark.bitrade.service.optfor.RedisSetService
...
### 六、zset数据类型操作
类名：com.spark.bitrade.service.optfor.RedisZSetService
...
### 三、Redis***Service使用
1.使用Redis***Service类方法如下：
```java
@Autowired
private Redis***Service redis***Service;
```
注意：使用前请确保能使用`@Autowired`自动注入RedisTemplate。

...
public class ThirdDESUtil {
	...
	public static String ENCRYPTMethod(String HexString, String keyStr)
		...
		try {
			byte[] theKey = null;
			theKey = SignUtil.md5Digest(keyStr).substring(0,8).getBytes(ENCODED_UTF8);
	...
	public static String getstrByte(String str){
		...
		byte[] digest;
		try {
			digest = str.getBytes(ENCODED_UTF8);
		} catch (UnsupportedEncodingException e) {
			throw new IllegalStateException(ENCODED_UTF8 + " not supported!");
		}
	...
	public static String ENCRYPTMethod(String HexString, String keyStr,String keyENCODED,String HexStringENCODED,String CipherInstanceType)
		...
		try {
			byte[] theKey = null;
			String jqstr = getstrByte(keyStr).toUpperCase();
	...
	public static String decrypt(String message, String key) throws Exception {
		...
		byte[] bytesrc = convertHexString(message);
		byte[] theKey = null;
		theKey = SignUtil.md5Digest(key).substring(0,8).getBytes(ENCODED_UTF8);
		Cipher cipher = Cipher.getInstance(CIPHER_INSTANCE_CBC);
		DESKeySpec desKeySpec = new DESKeySpec(theKey);
		SecretKeyFactory keyFactory = SecretKeyFactory.getInstance("DES");
		SecretKey secretKey = keyFactory.generateSecret(desKeySpec);

...
import com.spark.bitrade.mocker.config.ApiClientConfiguration;
import okhttp3.*;
...
public class ApiClient {
    ...
    String call(String method, String uri, Object object, Map<String, String> params) {
        ApiSignature sign = new ApiSignature();
        sign.createSignature(this.accessKeyId, this.accessKeySecret, method, API_HOST, uri, params);
        try {
            Request.Builder builder = null;
    ...
    static OkHttpClient createOkHttpClient() {
        if(ApiClientConfiguration.isProxy()){
            System.out.println(String.format("------启动提示：通过代理上网,host{%s},port{%d}-----",ApiClientConfiguration.getProxyHost(), ApiClientConfiguration.getProxyPort()));
            //指定代理上网 edit by yangch 2018-04-17
            return new Builder().connectTimeout(CONN_TIMEOUT, TimeUnit.SECONDS)
                    ...
                    .proxy(new Proxy(Proxy.Type.HTTP, new InetSocketAddress(ApiClientConfiguration.getProxyHost(), ApiClientConfiguration.getProxyPort())))
                    .build();
        } else {
            System.out.println("------启动提示：未通过代理上网--------------");
            return new Builder().connectTimeout(CONN_TIMEOUT, TimeUnit.SECONDS)
                    .readTimeout(READ_TIMEOUT, TimeUnit.SECONDS).writeTimeout(WRITE_TIMEOUT, TimeUnit.SECONDS)
